...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	/*  增加代码 检查 goods_url 是否重复_start   By thunje#URLdf */
    if ($_POST['define_url'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') .
                " WHERE define_url = '$_POST[define_url]'  AND goods_id <> '$_POST[goods_id]'";
        if ($db->getOne($sql) > 0)
        {
            sys_msg('您输入的URL已存在，请换一个', 1, array(), false);
        }
    }
	/* 增加代码 检查 goods_url 是否重复_end  By thunje#URLdf */
	...
	$define_url= !empty($_POST['define_url']) ? trim($_POST['define_url']) : '';  // by  thunje#URLdf
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
			/* 注意下面这两个SQL都加了一个 define_url 字段， by thunje#URLdf */
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, define_url, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$define_url', '$goods_sn', '$catgory_id', " .
...
/* 检查 define_url 唯一性_start By thunje#URLdf */
elseif ($_REQUEST['act'] == 'check_goods_url')
{
    check_authz_json('goods_manage');
    $goods_id = intval($_REQUEST['goods_id']);
    $define_url = json_str_iconv(trim($_REQUEST['define_url']));

    /* 检查是否重复 */
    if (!$exc->is_only('define_url', $define_url, $goods_id))
    {
        make_json_error('您输入的URL已存在，请换一个');
    }

    make_json_result('');
}
/* 检查 define_url 唯一性_end By thunje#URLdf */
